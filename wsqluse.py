...
class WSQLshell():
	...
	def save_db_txt(self, tablename):
		...
		for stringname in data:
			...
			#print('esc_date',esc_date)
			#carmodel = self.determineCarModel(stringname[0])
			#stringname.append(carmode)
	...
	def determineCarModel(self, carnum):
		ident = "Номер='{}'".format(carnum)
		carmodel = self.get_special_ident('Авто', 'Модель',ident)
		try:
			carmodel = carmodel[0][0]
		except IndexError:
			carmodel = 'Не опознано'
		print('Determined carmodel is', carmodel) 
		return carmodel
...
if __name__ == '__main__':
	sqlshell = WSQLshell('wdb', 'watchman', 'hect0r1337', 'localhost')
	#sqlshell.save_db_txt('ЖурналПосещений')
	#ident1 = "НомерАвто='у881ук102'"
	#ident2 = "НаТерритории='no'"
	#lv = sqlshell.get_last_visit('ЖурналПосещений', ident1, ident2)
	#print(lv)
	sqlshell.save_db_txt('ЖурналПосещений')
